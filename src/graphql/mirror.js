...
export class Mirror {
   ...
   *       comments: [
   *         {
   *           body: "I depend on this (see #1172); please do not change it.",
   *           author: {
  ...
  extract(rootId: Schema.ObjectId): mixed {
    ...
    return _inTransaction(db, () => {
      ...
      const temporaryTableName: string = db
        .prepare(
          dedent`\
            SELECT :prefix || (IFNULL(CAST(MAX(suffix) AS INTEGER), 0) + 1)
            FROM (
        ...
        .pluck()
        .get({prefix: "tmp_transitive_dependencies_"});
      // istanbul ignore if: should not be possible---it only has the
      // prefix (which is safe as defined above) and a trailing integer.
      if (!isSqlSafe(temporaryTableName)) {
        throw new Error(
          "Invariant violation: unsafe table name: " +
            JSON.stringify(temporaryTableName)
        );
      }
      db.prepare(
